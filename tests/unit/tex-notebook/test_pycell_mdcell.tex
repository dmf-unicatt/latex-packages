...
Implicit variable display also works when the instruction is split in multiple cells
\begin{pycell}
print(x + y + z + n - l - m + 1)
(x + y + z + n - l - m + 2)
\end{pycell}
...
\begin{pycell}
# fmt: off
z = "three THREE_Three"; n = "FoUr_four quattro_cuatro"  # noqa: E702
# fmt: on
\end{pycell}
...
# fmt: off
Z = '''three
THREE_Three'''  # noqa: Q001
N = '''FoUr_four
quattro_cuatro'''  # noqa: Q001
# fmt: on
print(f"{Z}\n{N}")
\end{pycell}
\begin{pyexpectedoutput}
...
fig
\end{pycell}
\pyexpectedfigure{images/scatter_html_latex_labels.png}
...
del fig  # fails with kaleido>=1, incorrect x axis
\end{pycell}
% \pyexpectedfigure{images/scatter_html_latex_labels_swapped.png}
...
\begin{pycell}
import plotly.graph_objects as go  # noqa: I001, RUF100
\end{pycell}

\begin{pycell}
def set_black_background(fig: go.Figure) -> None:
...
def check_image_size(
    image_paths: list[str], expected_dimensions: tuple[int, ...]
) -> None:
    ...
    for image_path in image_paths:
        ...
        assert abs(width - expected_width) <= expected_width_tolerance, (
            f"Image {image_path} has width "
            f"{width}, but expected width was "
            f"{expected_width} with tolerance "
            f"{expected_width_tolerance}"
        )
        assert abs(height - expected_height) <= expected_height_tolerance, (
...
\begin{pycell}
fig_path = os.path.join(  # ty: ignore[unresolved-reference]
    f"pythontex-files-{pytex.jobname}",  # ty: ignore[unresolved-reference]
    f"{pytex.id}_{pytex.instance}.png",  # ty: ignore[unresolved-reference]
)
...
check_image_size(
    [
        fig_path,
        os.path.join(  # ty: ignore[unresolved-reference]
            "images", "contour_plot_default_width_height_white_background.png"
        ),
    ],
    (1223, 6, 740, 6),  # affected by white space trimming
)
\end{pycell}
...
check_image_size(
    [
        fig_path,
        os.path.join(  # ty: ignore[unresolved-reference]
            "images", "contour_plot_default_width_height_black_background.png"
        ),
    ],
...
fig = go.Figure(data=[go.Contour(x=x_plot, y=y_plot, z=Z_plot)])
fig.update_layout(title="Contour plot", width=512, height=512, autosize=False)
fig
\end{pycell}
...
check_image_size(
    [
        fig_path,
        os.path.join(  # ty: ignore[unresolved-reference]
            "images", "contour_plot_fixed_width_height_white_background.png"
        ),
    ],
...
check_image_size(
    [
        fig_path,
        os.path.join(  # ty: ignore[unresolved-reference]
            "images", "contour_plot_fixed_width_height_black_background.png"
        ),
    ],
...
\begin{pycell}
old_plotly_default_width = pytex.plotly_default_width  # ty: ignore[unresolved-reference]
pytex.plotly_default_width = 555  # ty: ignore[unresolved-reference]
old_plotly_default_height = pytex.plotly_default_height  # ty: ignore[unresolved-reference]
pytex.plotly_default_height = 333  # ty: ignore[unresolved-reference]
old_plotly_default_scale = pytex.plotly_default_scale  # ty: ignore[unresolved-reference]
pytex.plotly_default_scale = 3  # ty: ignore[unresolved-reference]
\end{pycell}
...
check_image_size(
    [
        ...
        os.path.join(  # ty: ignore[unresolved-reference]
            "images",
            "contour_plot_new_default_width_height_white_background.png",
        ),
    ],
    (1396, 7, 608, 7),  # affected by white space trimming
)
\end{pycell}
...
check_image_size(
    [
        ...
        os.path.join(  # ty: ignore[unresolved-reference]
            "images",
            "contour_plot_new_default_width_height_black_background.png",
        ),
    ],
...
\begin{pycell}
pytex.plotly_default_width = old_plotly_default_width  # ty: ignore[unresolved-reference]
pytex.plotly_default_height = old_plotly_default_height  # ty: ignore[unresolved-reference]
pytex.plotly_default_scale = old_plotly_default_scale  # ty: ignore[unresolved-reference]
\end{pycell}
...
def save_cells_as_notebook(
    ...
    """Disable code cells containing pytex or fig_path."""
    modified_all_cells = []
    for cell in all_cells:
        if cell["type"] == "markdown":
            ...
            if "pytex" in cell["content"]:
                code = """
# fmt: off
print(
    'This cell was disabled because it contains the command pytex'  # noqa: Q000
)
# fmt: on"""
            elif "fig_path" in cell["content"]:
                code = """
# fmt: off
print(
    'This cell was disabled because it contains the variable fig_path'  # noqa: Q000
)
# fmt: on"""  # noqa: E501
            else:
                code = cell["content"]
            ...
            modified_cell["content"] = code
            modified_all_cells.append(modified_cell)

    return original_save_cells_as_notebook(modified_all_cells, pytex)
\end{pycell}
